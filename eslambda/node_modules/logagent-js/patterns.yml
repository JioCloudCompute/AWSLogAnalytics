...
- # Apache Solr
  blockStart: !!js/regexp \d+\s+\S{3,5}\s
...
- # Nova
  sourceName: !!js/regexp /nova/
  match:
    - type: nova_api
      regex: !!js/regexp (\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2}.\d+)\s+(\d+)\s+(\w+)\s+([\w.]+)\s+\[req-([\w-]+) - - - - -\]\s+([^/]+)\s+([/\w.-]+):(\d+)
      fields: [ts,pid,severity,path,request_id,message,service,line]
      #properties : {class: {type : string, index: not_analyzed}}
      dateFormat: YYYY-MM-DD HH:mm:ss.SSS
      transform: !!js/function >
        function (p) {
          p.raw_request_id = p.request_id.replace(/[-\/]/g,"_")
          p.raw_service = p.service.replace(/[-\/]/g,"_")
          p.raw_path = p.path.replace(/[-\/]/g,"_")
        }
    - type: jio_compute
      regex: !!js/regexp \[(\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2}.\d+)\]\s+(\w+)\:\s+([\w-_]+)\:\s+{"Severity"\:\s+(\d+),\s+"Service"\:\s+"([\w-_]+)",\s+"Metric Type"\:\s+"([\w-_]+)",\s+"Timestamp"\:\s+([\d.]+),\s+"Metric Value"\:\s+([\d.]+),\s+"Host"\:\s+"([\w-_.]+)",\s+"Metric Name"\:\s+"([\w-_]+)",\s+"request_id"\:\s+"req-([\w-]+)"}
      fields: [ts,severity,log_service,log_level,service,metric_type,timestamp,metric_value,host,metric_name,request_id]
      #properties : {class: {type : string, index: not_analyzed}}
      dateFormat: YYYY-MM-DD HH:mm:ss.SSSSSS
      transform: !!js/function >
        function (p) {
           p.raw_req = p.req.replace(/[-\/]/g,"_")
           p.raw_service = p.service.replace(/[-\/]/g,"_")
           p.raw_metric_name = p.metric_name.replace(/[-\/]/g,"_")
           p.raw_host = p.host.replace(/[-\/]/g,"_")
           p.raw_metric_type = p.metric_type.replace(/[-\/]/g,"_")
        }
...
- #  Web Logs
  ...
  match: 
    ...
    - regex:        !!js/regexp /(\S+)\s+\|\s(\S+)\s+([0-9a-f.:]+)\s+(-|.+?)\s+(-|.+?)\s\[(.*)\]\s"(.+?)\s(.+?)\s(.+?)"\s(\d+)\s(\d+)\s"(.+?)"\s"(.+?)"/i
      ...
      transform: !!js/function >
        function (p) {
          p.message = p.method + ' ' + p.virtual_host + p.path
        }
    # Common Log Format
