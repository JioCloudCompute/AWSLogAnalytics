...
- # Nova
  ...
  match:
    - type: nova_api
      regex: (\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2}.\d+)\s+(\d+)\s+(\w+)\s+([\w.]+)\s*\[([\w- ]*)\]\s+(.*)
      fields: [ts,pid,severity,path,request_id,message]
    ...
    - type: jio_compute
      regex: !!js/regexp \[(\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2}.\d+)\]\s+(\w+)\:\s+([\w-_]+)\:\s+{"Severity"\:\s+(\d+),\s+"Service"\:\s+"([\w-_]+)",\s+"Metric Type"\:\s+"([\w-_]+)",\s+"Timestamp"\:\s+([\d.]+),\s+"Metric Value"\:\s+([\d.]+),\s+"Host"\:\s+"([\w-_.]+)",\s+"Metric Name"\:\s+"([\w-_]+)",\s+"request_id"\:\s+"([\w- ]+)"}
